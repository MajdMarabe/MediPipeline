...
export async function processAction(actionType: string, payload: any) {
  switch (actionType) {
  ...
  setInterval(async () => {
    ...
    for (const job of jobs) {
      try {
        ...
        const result = await processAction(pipeline.actionType, job.payload);
        ...
        for (const sub of subscribers) {
          ...
          while (!success && attempts < MAX_ATTEMPTS) {
            ...
            console.log(x);
            if (!success) {
              console.log(`Retry ${attempts} failed for subscriber ${sub.url}`);
          ...
          if (!success) {
            allSuccess = false;
            console.error(`Failed after ${MAX_ATTEMPTS} attempts: ${sub.url}`);
          }
        }
...
/********** */
export async function sendToSubscriber(url: string, data: any) {
  try {
